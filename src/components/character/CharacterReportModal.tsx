...
function CharacterReportModal({
  ...
  const handleSubmit = useCallback(async () => {
    if (reportType === "stolen" && !link.trim()) return;
    if (reportType !== "stolen" && details.trim().length <= 5) return;
  ...
  }, [characterId, reportType, link, details, handleClose]);
          ...
          {phase === 1 ? (
            <>
              <View style={styles.header}>
                <Text style={styles.title}>Report Character</Text>
